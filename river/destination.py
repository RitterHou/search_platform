...
class ElasticSearch7Destination(DataDestination):
    """
    数据流目的地为Elasticsearch-7.5.2
    """
    ...
    def push(self, destination_config, data, param=None):
        ...
        self._add_private_field(es_config, data, input_param)
        if operation == 'create':
            es7_adapter.batch_create(es_config, data, input_param)
        elif operation == 'update':
            es7_adapter.batch_update(es_config, data, input_param)
        elif operation == 'delete':
    ...
    def clear(self, destination_config, data, param=None):
        ...
        if param:
            ...
            data = dict(data, **param)
        if clear_policy == 'every_msg,all':
            es7_adapter.delete_all_doc(es_config, data)
        elif clear_policy == 'every_msg,by_adminId':
            if not data.get('adminId'):
                app_log.error('destination clear fail, because adminId is null {0}', data)
                return

            es7_adapter.delete_by_field(es_config, data, '_adminId', data['adminId'])
...
DATA_DESTINATION_DICT = {
    'elasticsearch': ElasticSearchDestination(),
    'elasticsearch7': ElasticSearch7Destination()
}
