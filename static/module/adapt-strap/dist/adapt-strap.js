            ...
            var evalObjectProperty = function (obj, property) {
                ...
                if (filter) {
                    if (filter instanceof Array) {
                        for (var i in filter) {
                            var cur_filter = filter[i];
                            optionsIndex = cur_filter.indexOf(':');
                            if (optionsIndex > -1) {
                                filterName = cur_filter.substring(0, optionsIndex);
                                filterOptions = cur_filter.substring(optionsIndex + 1);
                                value = $filter(filterName)(value, filterOptions);
                            } else {
                                value = $filter(cur_filter)(value);
                            }
                        }
                    }
                    else {
